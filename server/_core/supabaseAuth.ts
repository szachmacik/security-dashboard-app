...
// Allowed emails — loaded from env var at runtime, fallback to static list
...
// Admin emails — loaded from env var at runtime
function getAdminEmails(): Set<string> {
  const csv = process.env.ADMIN_EMAILS_CSV;
  if (csv) {
    return new Set(csv.split(",").map((e) => e.trim().toLowerCase()).filter(Boolean));
  }
  return new Set(["maciej.koziej01@gmail.com", "manus@ofshore.dev"]);
}
